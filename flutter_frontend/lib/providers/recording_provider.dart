...
class RecordingProvider with ChangeNotifier {
  ...
  Future<void> stopRecording() async {
    ...
    try {
      ...
      if (result != null) {
        ...
        await _processRecording(result);
      } else {
        _setStatus(
            _isMonitoring ? RecordingStatus.monitoring : RecordingStatus.idle);
      }
    } catch (e) {
      _setError(e.toString());
      _setStatus(RecordingStatus.error);
  ...
  Future<void> _processRecording(Map<String, dynamic> uploadResult) async {
    try {
      final recording = Recording.fromJson(uploadResult['recording']);
  ...
  Future<void> fetchRecordings({int page = 1, int limit = 10}) async {
    try {
      _setError(null);
      final response =
          await _apiService.getRecordings(page: page, limit: limit);
